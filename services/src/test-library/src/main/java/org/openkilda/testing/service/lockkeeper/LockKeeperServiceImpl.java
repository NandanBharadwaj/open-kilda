...
public class LockKeeperServiceImpl implements LockKeeperService {
    ...
    public void knockoutSwitch(Switch sw) {
        ...
        Arrays.asList(mgmtFactory, statsFactory).forEach(floodlight -> floodlight.getRegions().forEach(region -> {
            ...
            restTemplatesPerRegion.get(region).exchange("/floodlight/block-switch", HttpMethod.POST,
    ...
    public void reviveSwitch(Switch sw) {
        ...
        Arrays.asList(mgmtFactory, statsFactory).forEach(floodlight -> floodlight.getRegions().forEach(region -> {
            String containerName = floodlight.getContainerName(region);
            restTemplatesPerRegion.get(region).exchange("/floodlight/unblock-switch", HttpMethod.POST,
                    new HttpEntity<>(new BlockRequest(containerName, swInfo.getAddress(), swInfo.getPort()),
                            buildJsonHeaders()), String.class);
        }));
    }
